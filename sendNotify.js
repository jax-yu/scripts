 ...
 * 1. 京东账号 N
 * 2. pt_pin 替换
 * process.env.NEED_REPLACE_ACCOUNT = `第一个账号:pt_pin
 * 第二个账号
 *
 * 第四个账号`
 */
...
async function sendNotify(text, desp, params = {}, author = "xajeyu@gmail.com") {
  ...
  const teamFlag = IMPORTA_NTNOTICE_SCRIPTS.some(item => text.match(item) !== null) || desp.match(/((已可)((兑换)|(领取))|(已集齐全部)|(保价成功)|(请重新登录获取)|(已达到))/)
